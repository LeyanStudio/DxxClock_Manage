...
if ($userrow['status'] == 1) {} else {header("Location:/Home.php");}
      ...
      <div id="appLogin">
        <div class="logins position" id="MainIndex" style="width: 700px;margin-left: -350px;height: 530px;">
          ...
          <div class="mip-reg-heading">【任务【ID - <?= $_GET['TID'] ?>】结算】
              <button type="button" class="el-button w-100 el-button--primary Bg-Greens" onclick="ClickSettleBtn('<?= $_GET['TID'] ?>','yes');" style="width:15%;height:30px;">
                <span>刷新数据</span>
              </button>
          </div>
          <div class="mip-reg-body">
            <div class="el-form-item is-required">
              <div class="el-form-item__content">
                <button type="button" class="el-button w-100 el-button--primary Bg-Blues" onclick="ClickSettleBtn('<?= $_GET['TID'] ?>');" style="float:left;width:30%;padding:10px;">
                  <span>一键结算</span>
                </button>
            ...
            <?
                ...
                $clocks = $DB->query("SELECT * FROM Web_Clocking WHERE taskbatch={$_GET['TID']}")->fetchAll();
            ?>
              <table style="width:100%;">
                  ...
                  <tbody id="TbodyList" style="text-align:center;">
                      <?
                      foreach ($clocks as $t) {
                          $user = $DB->query("SELECT * FROM Web_User WHERE id={$t['uid']}")->fetch();
                      ?>
                      <tr id="<?= $t['id'] ?>" stid="<?= $t['stid'] ?>">
                          <td><?= $t['id'] ?></td>
                          <td><?= $user['username'] ?></td>
                          <td><?= $user['class'] ?>班</td>
                          <td style="display:none;"><?= $res ?></td>
                          <td><?= ($t['addtime'] == '0000-00-00 00:00:00' ? '<b style="color:red;">未结算</b>' : $t['addtime']) ?></td>
                          <td>
                              <?
                              if ($t['status'] == 0) {
                              ?>
                              <button class="ClickBT Bg-Greens" type="settle" style="height:20px;" id="cid-<?= $t['id'] ?>" onclick="TaskSettle('<?= $_GET['TID'] ?>','<?= $t['id'] ?>');">获取是否完成学习</button>
                              <?
                              } elseif ($t['status'] == 3) {
                              ?>
                              <button class="ClickBT Bg-Reds" type="settle" style="height:20px;" id="cid-<?= $t['id'] ?>" onclick="TaskSettle('<?= $_GET['TID'] ?>','<?= $t['id'] ?>');">此用户未完成学习</button>
                              <?
                              } else {
                              ?>
                              <button class="ClickBT Bg-Blues" style="height:20px;">此用户已完成学习</button>
                              <?
                              }
                              ?>
                          </td>
