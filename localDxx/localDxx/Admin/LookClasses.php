...
<html>
          ...
          <div class="mip-reg-body">
            <div class="el-form-item is-required">
              <div class="el-form-item__content">
                <button type="button" class="el-button w-100 el-button--primary Bg-Blues" onclick="OpenAdminAddClass('<?= $_GET['GID'] ?>');" style="float:left;width:30%;padding:10px;">
                  <span>添加班级</span>
                </button>
                ...
                <button type="button" class="el-button w-100 el-button--primary Bg-Blues" style="float:left;width:25%;padding:10px;margin-left:5px;" onclick="LoadDiv('?GID=1');">
                  <span>刷新列表</span>
                </button>
              </div>
            </div>
            <input type="hidden" id="selclass" value="<?= $_GET['GID'] ?>" />
              ...
              <table style="width:100%;" id="TbodyTableList">
                  <thead>
                      <tr>
                          ...
                          ?>
                              <select id="ChangeTask" onchange="ChangeTask('Grade');"<?= (!$tasks ? 'disabled="disabled"' : '') ?>>
                  ...
                  <tbody id="TbodyList" style="text-align:center;">
                      ...
                      foreach ($classes as $c) {
                          $ms = $thisDB->query("SELECT * FROM Web_User WHERE class='{$c['name']}'")->fetchAll();
                          $counts = count($ms);
                          $user = $thisDB->query("SELECT * FROM Web_User WHERE id='{$c['lid']}' limit 1")->fetch();
                          
                          if ($_GET['TID'] == null) {
                              $task = $thisDB->query("SELECT * FROM Web_Task ORDER BY id DESC limit 1")->fetch();
                          } else {
                              $task = $thisDB->query("SELECT * FROM Web_Task WHERE id='{$_GET['TID']}' limit 1")->fetch();
                          }
                          $tid = $task['id'];
                          if (!$task) {
                              $tres = '无此打卡任务';
                              ...
                              foreach ($cis as $ci) {
                                  if ($thisDB->query("SELECT * FROM Web_User WHERE id='{$ci['uid']}' and class='{$c['name']}' limit 1")->fetch()) {
                                      $cs[] = $ci;
                                  }
                              }
                              $cisc = @count($cs);
                              if (!$cisc || $cisc == 0) {
                                  ...
                                  /*if (time() >= $task['endtime']) {
                                      if ($thisDB->query("SELECT count(*) FROM Web_Clocking WHERE class='{$c['name']}' and status='0'")->fetchColumn() == 0) {
                                          $unclmems = $thisDB->query("SELECT count(*) FROM Web_Clocking WHERE taskbatch='{$tid}' and class='{$c['name']}' and status='3'")->fetchColumn();
                                      } else {
                                          $unclmems = $thisDB->query("SELECT count(*) FROM Web_User WHERE class='{$c['name']}' and username!='Administrator'")->fetchColumn() - $cisc;
                                      }
                                      
                                      $hcs = $thisDB->query("SELECT count(*) FROM Web_Clocking WHERE taskbatch='{$tid}' and class='{$c['name']}' and status='2'")->fetchColumn();
                                      
                                      $tres = '<b style="color:blue;">【'.($cisc - $hcs).'】</b>人已打卡<br><b style="color:orange;">【'.$hcs.'】</b>人谎打卡<br><b style="color:red;">【'.$unclmems.'】</b>人未打卡';
                                      
                                      $cs = null;
                                      
                                      $cis = $thisDB->query("SELECT * FROM Web_Clocking WHERE taskbatch='{$tid}' and status='1'")->fetchAll();
                                      foreach ($cis as $ci) {
                                          if ($thisDB->query("SELECT * FROM Web_User WHERE id='{$ci['uid']}' and class='{$c['name']}' limit 1")->fetch()) {
                                              $cs[] = $ci;
                                          }
                                      ...
                                      if ($thisDB->query("SELECT count(*) FROM Web_Clocking WHERE taskbatch='{$tid}' and class='{$c['name']}' and status='3'")->fetchColumn() != 0) {
                                          $unclmems = $thisDB->query("SELECT count(*) FROM Web_Clocking WHERE taskbatch='{$tid}' and class='{$c['name']}' and status='3'")->fetchColumn();
                                      } else {
                                          $unclmems = $thisDB->query("SELECT count(*) FROM Web_User WHERE class='{$c['name']}' and username!='Administrator'")->fetchColumn() - $cisc;
                                      }
                      ...
                      <tr id="<?= $c['id'] ?>" cname="<?= $c['name'] ?>">
                          ...
                          <td><?= $c['id'] ?></td>
                          <td><?= $grade['name'].$c['name'] ?>班</td>
                          <td class="CanClick" onclick="window.location.href = 'LookMembers.php?GID=<?= $grade['gradeid'] ?>&CID=<?= $c['name'] ?>&TYPE=1';"><?= ($counts == null ? '0' : $counts) ?>人</td>
                          <td id="<?= $c['name'] ?>-lid"><?= ($c['lid'] == 0 ? '暂无' : '<b style="color:red;">'.$user['username'].'</b>') ?></td>
                          <td><?= $tres ?></td>
                          <td style="color:<?= (!$cisc || $cisc == 0 ? 'red' : 'green') ?>;"><?= (!$cisc || $cisc == 0 ? '无人打卡' : substr(($cisc/($thisDB->query("SELECT count(*) FROM Web_User WHERE class='{$c['name']}' and username!='Administrator'")->fetchColumn()))*100,0,6).'%') ?></td>
                          <td>
                              <button class="ClickBT Bg-Blues" style="height: 20px;" onclick="AdminSetAdmin('<?= $c['name'] ?>','<?= $_GET['GID'] ?>');">设置团支书</button>
                              <button class="ClickBT Bg-Greens" style="height: 20px;" onclick="AdminLookInfo('<?= $c['name'] ?>','<?= $_GET['GID'] ?>');">查看详细信息</button><br>
                              <button class="ClickBT Bg-Primary" style="height: 20px;margin-top: 6px;" onclick="AdminEditClass('<?= $c['id'] ?>','<?= $_GET['GID'] ?>');">编辑班级信息</button>
                              <button class="ClickBT Bg-Reds" style="height: 20px;margin-top: 6px;" onclick="AdminDelete('<?= $c['id'] ?>','<?= $_GET['GID'] ?>');">删除</button>
                          </td>
                      </tr>
                      <?
                      }
            ...
            <div style="width:100%;margin-left:10px;">
                ...
                <select id="AjaxDoType" gid="<?= $_GET['GID'] ?>" onchange="AjaxDoType();" style="height:20px;float: left;margin-left: 20px;margin-top: 1.5px;">
                    <option value="0">请选择批量操作方式</option>
                    <option value="AdminDelete">批量删除班级</option>
                </select>
            </div>
            <p class="login-link">
  ...
  if($_GET['TYPE'] == 1) {
  ?>
  //history.replaceState(null, null, 'LookClasses.php');
  <?
  }
  ?>
  </script>
