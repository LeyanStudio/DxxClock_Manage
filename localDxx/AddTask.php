      ...
      <div id="appLogin">
        <div class="logins position" id="MainIndex" style="width: 700px;margin-left: -350px;height: 530px;">
          ...
          <div class="mip-reg-heading">任务管理
              <button type="button" class="el-button w-100 el-button--primary Bg-Greens" onclick="LookTasksInfo();" style="width:15%;height:30px;">
                <span>查看概览图</span>
              </button>
          </div>
          <div class="mip-reg-body">
            <div class="el-form-item is-required">
              <div class="el-form-item__content">
                ...
                if ($userrow['status'] == 1) {
                ?>
                <button type="button" class="el-button w-100 el-button--primary Bg-Blues" onclick="AddTask();" style="float:left;width:30%;padding:10px;">
                  <span>添加任务</span>
                </button>
                <?
                }
                ?>
                <input type="text" oninput="SearchClass('2');" placeholder="请输入任务名称" class="el-input__inner" name="class" style="float:left;width:130px;margin-left:<?= ($userrow['status'] == 1 ? 5 : 55) ?>px;">
              ...
              <table style="width:100%;">
                  <thead>
                      <tr>
                          ...
                          <th style="width:<?= ($userrow['status'] == 1 ? '17' : '32') ?>%;"><?= ($userrow['status'] == 1 ? '总' : '本班') ?>打卡人数</th>
                  ...
                  <tbody id="TbodyList" style="text-align:center;">
                      ...
                      foreach ($tasks as $t) {
                          if ($userrow['status'] == 1) {
                              $cis = $DB->query("SELECT * FROM Web_Clocking WHERE taskbatch='{$t['id']}' and status!='3'")->fetchAll();
                              $us = $DB->query("SELECT * FROM Web_User WHERE username!='Administrator'")->fetchAll();
                              $hcs = $DB->query("SELECT * FROM Web_Clocking WHERE taskbatch='{$t['id']}' and status='2'")->fetchAll();
                          } else {
                              foreach ($DB->query("SELECT * FROM Web_Clocking WHERE taskbatch='{$t['id']}' and status!='3'")->fetchAll() as $c) {
                              ...
                              //获取谎打卡人数
                              foreach ($DB->query("SELECT * FROM Web_Clocking WHERE taskbatch='{$t['id']}' and status='2'")->fetchAll() as $hc) {
                                  if ((($DB->query("SELECT * FROM Web_User WHERE id='{$hc['uid']}' limit 1")->fetch())['class']) == $userrow['class']) {
                                    $hcs[] = $hc;
                                  }
                              }
                              
                              //获取用户数量
                              $us = $DB->query("SELECT * FROM Web_User WHERE username!='Administrator' and class='{$userrow['class']}'")->fetchAll();
                          }
                          $counts = @count($cis);
                          $uscounts = @count($us);
                          $hcounts = @count($hcs);
                          ...
                          if ($counts == null) {
                              ...
                              if ($DB->query("SELECT count(*) FROM Web_Clocking WHERE taskbatch='{$t['id']}' and status='3'")->fetchColumn() != 0) {
                                if ($userrow['status'] == 1) {
                                    $unclmems = $DB->query("SELECT count(*) FROM Web_Clocking WHERE taskbatch='{$t['id']}' and status='3'")->fetchColumn();
                                } else {
                                    $unclmems = $DB->query("SELECT count(*) FROM Web_Clocking WHERE taskbatch='{$t['id']}' and class='{$userrow['class']}' and status='3'")->fetchColumn();
                                }
                              } else {
                      ...
                      <tr id="<?= $t['id'] ?>" stid="<?= $t['stid'] ?>">
                          ...
                          <td>
                              ...
                              <button class="ClickBT Bg-Blues" onclick="AddTask('<?= $t['id'] ?>','Edit');">编辑</button>
                              <button class="ClickBT Bg-Reds" onclick="DeleteTask('<?= $t['id'] ?>');">删除</button>
                          </td>
                          <?
                          }
                          ?>
            ...
            <div style="width:100%;margin-left:10px;">
                ...
                <select id="AjaxDoType" onchange="AjaxDoType();" style="height:20px;float: left;margin-left: 20px;margin-top: 1.5px;">
                    ...
                    if ($userrow['status'] == 1) {
                    ?>
                    <option value="DeleteTask">批量删除任务</option>
                    <?
                    }
                    ?>
                </select>
