...
$dltzs = $thisDB->query("SELECT * FROM Web_User WHERE class='{$cid}' and status=2")->fetchAll();
foreach ($dltzs as $d) {
    if ($d['id'] != $class['lid']) {
        $dd[] = $d;
    }
...
$user = $thisDB->query("SELECT * FROM Web_User WHERE id='{$class['lid']}' limit 1")->fetch();
?>
<style>
.CIIndex {
    width: 100%;
...
<div class="CIIndex">
    ...
    <div style="margin-top: 10px;">
        ...
        <?
            ...
            foreach ($tasks as $t) {
                $clocking = $thisDB->query("SELECT * FROM Web_Clocking WHERE class='{$cid}' and taskbatch='{$t['id']}' and status!='3'")->fetchAll();
                $hcs = $thisDB->query("SELECT * FROM Web_Clocking WHERE taskbatch='{$t['id']}' and class='{$cid}' and status='3'")->fetchAll();
        ?>
        <?
        if (time() >= $t['endtime']) {
            $hcounts = @count($hcs);
