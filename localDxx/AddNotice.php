      ...
      <div id="appLogin">
        <div class="logins position" id="MainIndex" style="width: 700px;margin-left: -350px;height: 530px;">
          ...
          <div class="mip-reg-heading">公告管理
              <button type="button" class="el-button w-100 el-button--primary Bg-Greens" onclick="LookNoticesInfo();" style="width:15%;height:30px;">
                <span>查看概览图</span>
              </button>
         </div>
          <div class="mip-reg-body">
            <div class="el-form-item is-required">
              <div class="el-form-item__content">
                <button type="button" class="el-button w-100 el-button--primary Bg-Blues" onclick="OpenAddNotice();" style="float:left;width:30%;padding:10px;">
                  <span>添加公告</span>
                </button>
              ...
              <table style="width:100%;">
                  ...
                  <tbody id="TbodyList" style="text-align:center;">
                      ...
                      foreach ($notices as $n) {
                          //$counts = @count(explode(',',$n['status']));
                          $counts = count($DB->query("SELECT * FROM Web_NoticeLooked WHERE nbatch='{$n['id']}' and uid!='1'")->fetchAll());
                              
                          //获取用户数量
                          foreach ($DB->query("SELECT * FROM Web_User WHERE username!='Administrator'")->fetchAll() as $u) {
                              $us[] = $u;
                          }
                      ?>
                      <tr id="<?= $n['id'] ?>">
                          ...
                          <td><?= $n['id'] ?></td>
                          <td><?= $n['title'] ?></td>
                          <td><button class="ClickBT Bg-Blues" title="点击查看公告内容" style="height: 25px;width: 70px;" onclick="LookNoticeMsg('<?= $n['id'] ?>');">点击查看</button><msg id="m-<?= $n['id'] ?>"><?= str_replace('<br>',"\n",$n['msg']) ?></msg></td>
                          <td><?= $n['addtime'] ?></td>
                          ...
                          <td>
                              <button class="ClickBT Bg-Blues" style="height: 28px;" onclick="ErrorMsg('此公告为总管理所发布，您无权限操作！');">编辑</button>
                              <button class="ClickBT Bg-Reds" style="height: 28px;" onclick="ErrorMsg('此公告为总管理所发布，您无权限操作！');">删除</button>
                              <?
                              } else {
                              ?>
                          <td class="CanClick" onclick="LookLookingMen('<?= $n['id'] ?>');"><?= $counts ?>人</td>
                          <td style="color:<?= ($counts == 0 ? 'red' : 'green') ?>;"><?= ($counts == 0 ? '无人查看' : substr(($counts/@count($us))*100,0,6).'%') ?></td>
                          <td>
                              <button class="ClickBT Bg-Blues" style="height: 28px;" onclick="OpenEditNotice('<?= $n['id'] ?>');">编辑</button>
                              <button class="ClickBT Bg-Reds" style="height: 28px;" onclick="DeleteNotice('<?= $n['id'] ?>');">删除</button>
                              <?
                              }
                              ?>
                          </td>
            ...
            <div style="width:100%;margin-left:10px;">
                ...
                <select id="AjaxDoType" onchange="AjaxDoType();" style="height:20px;float: left;margin-left: 20px;margin-top: 1.5px;">
                    ...
                    <option value="SetNoticeLT">批量设置查看对象</option>
                    <option value="DeleteNotice">批量删除公告</option>
                </select>
            </div>
            <p class="login-link">
